...
function GetWindowsMessage(const MessageId: DWORD; const Module: string; const Args: array of string): string;
...
function FormatMessageFromSystem(const MessageId: DWORD; const AddId: boolean = false; const Module: string = ''): string;
  ...
  if Module <> '' then
    begin
    ModuleHandle := LoadLibraryEx(pchar(Module),              // LPCSTR lpLibFileName
                                  0,                          // HANDLE hFile
